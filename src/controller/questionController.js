...
const questionCreateUpdate = catchAsync(async (req, res) => {
  ...
  if (id) {
    ...
    await question.save().then((response) => {
      return responseSend(res, 200, "Question updated successfully", response);
    });
  } else {
    await QuestionModel.create({
      ...
      return responseSend(res, 201, "Question created successfully", response);
    });
  }
});

const questionDelete = catchAsync(async (req, res) => {
...
const questionDetail = catchAsync(async (req, res) => {
  const { questionId } = req.params;
...
const submitAnswer = catchAsync(async (req, res) => {
  ...
  // const correctAnswer = validQuestion.correctAnswer;
  // if (correctAnswer !== answer) {
  //   return responseSend(res, 400, "Answer is incorrect");
  // }
  ...
  if (alreadySubmitted > 0) {
    return responseSend(
      res,
      400,
      "You are already submitted the answer for this question"
    );
  }
